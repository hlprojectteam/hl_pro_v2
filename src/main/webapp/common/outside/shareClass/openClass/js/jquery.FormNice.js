...
$('select').each(function(){
	...
	if($(this).hasClass('cascade')){cas='ui-select-cascade'};
	...
	for(var i=0;i<=olen;i++){
		if($opt.eq(i).attr('data-tier') != undefined){
			tier='class="tier-'+$opt.eq(i).attr('data-tier')+'"';
		}else{
			tier='';
		};
		oStr=oStr+'<a href="#" '+tier+'data-val="'+$opt.eq(i).val()+'">'+$opt.eq(i).text()+'</a>';
	};
	var oTxt,oVal;
	if(place == undefined){
		oVal = $opt.eq(0).val();
	...
	var myStr='<span class="ui-select-inner"><input type="text" class="ui-select-txt" '+readonly+' placeholder="请选择" /><i></i></span><span class="ui-option-wrap"><span class="ui-option">'+oStr+'</span></span>';
	$(this).hide().wrap('<span class="'+sClass+'" style="'+sStyle+'"></span>');
...
$('.ui-option a').click(function(e){
	...
	if ($(this).hasClass('tier-1')) {
		//$(this).parents('.ui-select').addClass('ui-select-open').children('.ui-option-wrap').show();
		alert('请选择分类');
	}else{
		var txt=$(this).text();
		...
		$(this).parents('.ui-select').removeClass('ui-select-open').children('.ui-option-wrap').hide();
	};
});
//----------=E 模拟下拉框 End ----------//
...
//模拟复选框
$("body").on("click" ,'.ui-checkbox:not(".ui-checkbox-all,.ui-checkbox-disabled,.ui-checkbox-disabled-checked")',function(e){
	e.preventDefault();
	var chkLen=$(this).parents('form').find('.ui-checkbox').not('.ui-checkbox-all').length;
	if(!$(this).hasClass('ui-checkbox-current')){
		$(this).addClass('ui-checkbox-current').find('.chk').prop('checked','checked');
		var chkedLen=$(this).parents('form').find('.ui-checkbox-current').not('.ui-checkbox-all').length;
		if(chkLen==chkedLen){
			$(this).parents('form').find('.ui-checkbox-all').addClass('ui-checkbox-current').find('.chk').prop('checked','checked');
...
$('.ui-checkbox-all').click(function(){
	...
	if(chkAll=="no"){
		$(this).attr('data-all','yes').parents('form').find('.ui-checkbox').addClass('ui-checkbox-current').find('.chk').prop('checked','checked');
	}else{

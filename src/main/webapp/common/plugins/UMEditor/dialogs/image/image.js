...
//                }
    ...
    var Upload = {
       ...
       //     url=url + (url.indexOf("?") == -1 ? "?" : "&") + "editorid="+me.editor.id;//初始form提交地址;
            url=url + (url.indexOf("?") == -1 ? "?" : "&") + "formId="+me.editor.getOpt("id");//初始form提交地址;
        ...
        uploadComplete: function(r){
            var me = this;
            try{
            	r = r.substring(r.indexOf("{"), end = r.indexOf("}")+1);//由于返回字符串含有<prev>,所以使用此方法去掉标签
                var json = eval('('+r+')');
...
//                            xhr.open("post", me.editor.getOpt('imageUrl') + "?type=ajax", true);
                            xhr.open("post", me.editor.getOpt('imageUrl') + "?formId="+me.editor.getOpt("id"), true);

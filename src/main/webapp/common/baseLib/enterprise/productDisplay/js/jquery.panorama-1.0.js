...
jQuery.fn.threesixty = function(options){
	...
	*/
	...
	return this.each(function(){
		...
		$.each(options.images, function(index, record) { var o =$("<img>").attr("src",record).hide();	$(".panorama_box").append(o); });
	...
	function initTools(imgEle){
		...
		$(".turnleftNew").click(function(){
			//pause(1);
			autoPlayNew(_this, "left");
		});
		
		$(".turnrightNew").click(function(){
			//pause(1);
			autoPlayNew(_this, "right");
		});	
	...
	*/
	...
	function autoPlayNew(imgEle, dire){
		//clearTimer();
		direction = dire || "right"; //default direction is right.
		if(isFirst) {
			index = (direction == "right" ? 0 : options.images.length - 1);		

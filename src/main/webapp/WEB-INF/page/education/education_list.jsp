		...
		<form id="baseForm" method="post" name="baseForm" class="form-horizontal" action="" >
		 <input type="hidden"  id="type" name="type" value="${type}" />
		  <div class="row">
		    ...
		    <div class="col-sm-2">
		      	 <opt:select dictKey="Education_Category" classStyle="form-control" id="category" name="category" isDefSelect="true"/>
		    </div>
    ...
    function loadGrid() {
    	$("#grid").bootstrapTable({
    	    ...
    	    columns: [{checkbox:true},
    	              ...
    	              {title: "作者名", field: "authorName",width: 80, align:"center"},
    	              {title: "分类", field: "category", width: 80,align:"center",formatter:function(value,row,index){
		           	  		return changeDataDictByKey("Education_Category",value);
		              }},
    	              {title: "发布时间", field: "releaseDate", width: 100,align:"center",formatter:function(value,row,index){
   	              	  ...
   	              	  {title: "点击量", field: "visitNum",width: 50, align:"center"},
  	...
  	function on_add(){
  		parent.layer.open({
            ...
            area: ["80%", "100%"],
            content: "/education/education_edit?type="+$("#type").val()
        });
  	}
  		...
  		if(rows.length>0){
  			parent.layer.confirm("确定删除所选数据？", {
				...
				$.ajax({
					...
					async:false,
					dataType : "json",
					url: "/education/education_delete?ids="+ids,
					success : function(data){
						if(data.result=="success"){
							$("#grid").bootstrapTable("refresh",{url:"/education/education_load"});//加载树下的列表
							autoMsg("删除成功！",1);
						}else{
							autoMsg("删除失败！",1);
						}
  	...
  	function on_edit(obj){
  		parent.layer.open({
            ...
            maxmin: true, //开启最大化最小化按钮
            area: ["80%", "100%"],
            content: "/education/education_edit?id="+obj.id+"&type="+$("#type").val()
        });
  	}
...
function changeState(obj){
	$.ajax({
		...
		async:false,
		dataType : 'json',
		url: '/education/education_changeState?id='+obj.id,
		success : function(data){
			if(data.result){
				if(data.sign=="up")
					autoMsg("发布成功！",1);

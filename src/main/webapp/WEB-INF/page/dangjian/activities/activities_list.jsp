    ...
    function loadGrid() {
    	$("#grid").bootstrapTable({
    	    ...
    	    columns: [{checkbox:true},
		              ...
		              {title: "活动分值",field: "points",width: 80,align:"center"},
  	...
  	function on_add(){
  		parent.layer.open({
            ...
            area: ["80%", "80%"],
            content: "/dangjian/activities_edit?"
        });
  	}
  		...
  		if(rows.length>0){
  			parent.layer.confirm("确定删除所选数据？", {
				...
				$.ajax({
					...
					async:false,
					dataType : "json",
					url: "/dangjian/activities_del?ids="+ids,
					success : function(data){
						if(data.result){
							$("#grid").bootstrapTable("refresh",{url:"/dangjian/activities_load"});//加载树下的列表
							autoMsg("删除成功！",1);
						}else{
							autoMsg("删除失败！",1);
						}

...
<input type="hidden" id="cover" name="cover" value="${activitiesVo.cover}" />
<input type="hidden" id="id" name="id" value="${activitiesVo.id}" />	
<input type="hidden" id="createTime" name="createTime" value="<fmt:formatDate value='${activitiesVo.createTime}'  pattern='yyyy-MM-dd HH:mm:ss'/>"/>
<input type="hidden" id="creatorId" name="creatorId" value="${activitiesVo.creatorId}" />	
<input type="hidden" id="creatorName" name="creatorName" value="${activitiesVo.creatorName}" />		
	<%-- 第1行 --%>
	<div class="form-group">
	  	<label class="col-sm-2 control-label"><span style="color: red">*</span>活动标题</label>
	    <div class="col-sm-10">
	      <input type="text" class="form-control" id="title" name="title" placeholder="输入活动标题" value='${activitiesVo.title}' data-rule-required="true" data-rule-rangelength="[1,50]" />
	    </div>
  	</div>
  	...
  	<div class="form-group">
	    ...
	    <div class="col-sm-3">
	      <opt:select dictKey="dj_activities_frequency" classStyle="form-control" id="frequency" name="frequency"  value="${activitiesVo.frequency}" />
	    </div>
	    
	    <label class="col-sm-2 control-label"><span style="color: red">*</span>排序</label>
	    <div class="col-sm-3">
			<input type="number" class="form-control" id="order" name="order" value='${activitiesVo.order}' data-rule-required="true" data-rule-rangelength="[1,2]" />    
		</div>
    </div>
    ...
    <%-- 第3行 --%>
  	<div class="form-group">
	    ...
	    <div class="col-sm-3">
			<input type="text" class="form-control" id="points" name="points" placeholder="积分值为1-100之间" value='${activitiesVo.points}' data-rule-required="true" data-rule-rangelength="[1,2]" oninput="value=value.replace(/[^\d]/g,'')"/>    
		</div>
	</div>
	...
	<div class="form-group">
	    ...
	    <div class="col-sm-10">
	       <textarea class="form-control" rows="4" cols="" id="content" name="content" placeholder="描述你的活动，长度为1000字"  data-rule-required="true" data-rule-rangelength="[1,1000]" >${activitiesVo.content}</textarea>
	    </div>
  	</div>
...
$().ready(function() {
	var id = '${activitiesVo.id}';
	if(id!=null&&id!=""){
		getAttach(id,'dj_activities_cover');//获得照片

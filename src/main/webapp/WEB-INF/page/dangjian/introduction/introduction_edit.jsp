...
<input type="hidden" id="cover" name="cover" value="${introductionVo.cover}" />
<input type="hidden" id="status" name="status" value="${introductionVo.status}" />
<input type="hidden" id="visitNum" name="visitNum" value="${introductionVo.visitNum}" />
<input type="hidden" id="id" name="id" value="${introductionVo.id}" />		
<input type="hidden" id="createTime" name="createTime" value="<fmt:formatDate value='${introductionVo.createTime}'  pattern='yyyy-MM-dd HH:mm:ss'/>"/>
<input type="hidden" id="creatorId" name="creatorId" value="${introductionVo.creatorId}" />	
<input type="hidden" id="creatorName" name="creatorName" value="${introductionVo.creatorName}" />
<input type="hidden" id="category" name="category" value="${introductionVo.category}" />
...
<!-- <span id="noDelPhotoSign"></span> -->
    ...
    <div class="col-sm-10">
      <input type="text" class="form-control" id="mainTitle" name="mainTitle" value='${introductionVo.mainTitle}' data-rule-required="true" data-rule-rangelength="[1,100]" />
    </div>
  	</div>
     ...
     <div class="col-sm-3">
      <input type="text" class="form-control" id="authorName" name="authorName" value="${introductionVo.authorName}"  data-rule-rangelength="[1,20]" />
     </div>
    </div>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-3">
            <opt:select dictKey="news_ShowPlace" classStyle="form-control" id="showPlace" name="showPlace" value="${introductionVo.showPlace}" />
        </div>
        <label class="col-sm-2 control-label"><span style="color: red">*</span>是否置顶</label>
        <div class="col-sm-3">
            <opt:select dictKey="isNot" classStyle="form-control" id="isTop" name="isTop" value="${(not empty introductionVo.isTop) ? introductionVo.isTop : 0}" />
        </div>
    </div>
  	...
  	<div class="form-group">
        ...
        <div class="col-sm-3">
            <opt:select dictKey="news_Lable" classStyle="form-control" id="lable" name="lable" value="${introductionVo.lable}" isDefSelect="true" />
        </div>
	 
	      <label class="col-sm-2 control-label"><span style="color: red">*</span>来源</label>
	    <div class="col-sm-3">
	      <opt:select dictKey="news_Source" classStyle="form-control" id="newSource" name="newSource" value="${introductionVo.newSource}" />
	    </div>
	</div> 
	...
	<div class="form-group">
        ...
        <div class="col-sm-3">
            <input type="text" class="form-control" id="releaseDateStr" name="releaseDateStr" value="<fmt:formatDate value='${introductionVo.releaseDate}' pattern='yyyy-MM-dd HH:mm:ss'/>" data-rule-required="true" onfocus="this.blur()" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>    
        </div>
    </div>
    ...
    <div class="col-sm-10">
			...
			<script type="text/javascript">
			    ...
			    editor.ready(function() {
			        editor.setContent('${introductionVo.contentStr}');//设置编辑器的内容
			    });
			</script>
    </div>
  	</div>
...
<script type="text/javascript">
var mType;//简介类型
var mBranchId;//当前支部ID
$().ready(function() {
	var id = '${introductionVo.id}';
	mType  = '${introductionVo.category}';
	mBranchId  = '${introductionVo.branchId}';
	if(id!=null&&id!=""){
		getAttach(id);//获得照片
		queryAttachByType('dj_intro_file');
	}
});
//新增保存更新
function on_save(){
...
function on_submit(){  
	...
	$.ajax({
		...
		async:false,
		dataType : 'json',
		url: '/dangjian/introduction_saveOrUpdate',
		data:$('#baseForm').serialize(),
		success : function(data){
			if(data.result){
			    if(uploader.getFiles().length>0&&uploaderFile.getFiles().length>0){//检查上传对列是否有照片 和 附件
					uploader.options.server = '/attach_upload?formId='+data.id +'&attachType=dj_intro_cover';//自动添加水印
					uploader.upload();//上传
					uploader.on( 'uploadFinished', function( file ) {
					    setEntityImgPath(data.id);
					    
						uploaderFile.options.server = '/attach_upload?formId='+data.id +'&attachType=dj_intro_file';
					...
					uploader.options.server = '/attach_upload?formId='+data.id +'&attachType=dj_intro_cover';
					uploader.upload();//上传
...
function getAttach(id){
	$.ajax({
		...
		async:false,
		dataType : 'json',
		url: '/queryAttachListByType?formId='+id+'&type=dj_intro_cover',
		success : function(data){
...
function setEntityImgPath(id){
	$.ajax({
		...
		async:false,
		dataType : 'json',
		url: '/dangjian/update_introduction_img?id='+id,
		success : function(data){
		}
	});
}
